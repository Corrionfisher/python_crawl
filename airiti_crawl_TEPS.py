...
search_name = str('\"臺灣教育長期追蹤資料庫\" OR [ALL]:\"Taiwan Education Panel Survey\" OR [ALL]:\"TEPS\"') #本篇為: TEPS


# 設定參數2,3: 設定叢集名稱、搜尋來源

pName = str('TEPS') # 叢集名稱(會變成csv檔的名字，盡量用英文簡稱)
...
def crawlerPage(num, category, pName): # 2-1.先進入頁面
    ...
    category_text = soup.find('div', {'id':'menu'}).find('ul').find('li', {'id':category}).text
    pattern = '[0-9]+'
    category_re = re.findall(pattern, category_text)[0]  # 利用正規表達式抓出文字串的數字
    category_int = int(category_re) #當前共有幾筆資料

    if category_int == 0: # 2-3-1.發現當前頁面沒有資料
        print(category)
        print('共 0 筆')
        print('...........分隔線..........')
